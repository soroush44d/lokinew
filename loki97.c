...
static ULONG64 lookup_p(unsigned int index)
{
    ...
    value.r = 0;
    for (i = 0; i < 0x100; i++) {
        LOKI97_U32 mask = (LOKI97_U32)(0U - ct_eq_u32(i, index));
        sc_trace_p[i] = 1;
        value.l |= P[i].l & mask;
        value.r |= P[i].r & mask;
    }
...
{
    for (int j = 0; j < len; j++) {
        if (!isxdigit(hex[2 * j]) || !isxdigit(hex[2 * j + 1])) {
            printf("Invalid hex input\n");
            return NULL;
        }

        buf[j] = (BYTE)((fromHex(hex[2 * j]) << 4) | fromHex(hex[2 * j + 1]));
    }
...
int self_test(char* hexkey, char* hexplain)
{
    ...
    /* این مقدار هاردکد شده حذف می‌شود چون از ورودی می‌آید */
    ...
    /* تبدیل ورودی هگز plain به آرایه بایت */
    ...
    /* مقایسه با سایفر از پیش تعیین شده */
